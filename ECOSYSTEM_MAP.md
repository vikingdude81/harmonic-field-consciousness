...
|---|---|---|
| **Gabriel's Horn** (entropy-decay temperature schedule) | **RESOLVED & HARDENED** | `E:\horn-experiment` → github.com/vikingdude81/horn-experiment. Full implementation (6 processor variants incl. selective suppression), computed generation results, **unit tests (10 passing)**, and **Answerability Battery wiring** (`run_answerability.py`) — the first probe with measured A1/A2 results (modulation signature confirmed; see PROBE_MATRIX.md). Earlier loose copies in `E:\files (2)` are superseded. |
